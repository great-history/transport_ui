...
class My_Ametek7270(pymeasure.instruments.ametek.Ametek7270):
    def __init__(self, Ametek7270_addr: str, lockin_name: str, voltage: float, phase: float, frequency: float, interval_pause: float,
                 terminal_io: list, I_ac: float, check_ch: str, tol: float, path_list: list, flag_back: bool = True, harmonic_mode: int = 1, flag_init:bool = True):
        ...
        if flag_init:  ########## flag_init : 是否对仪器进行初始化, 如果要和仪器通讯, 则要进行初始化
            super().__init__(Ametek7270_addr)
            self.addr = Ametek7270_addr
            self.init_Ametek7270(voltage, phase, frequency, harmonic_mode)
        else:  ########## 加载数据
            if self.flag_back:
    ...
    def init_Ametek7270(self, voltage, phase, frequency, harmonic_mode):
        ####################################################################################################
        #################### Ametek7270参数设置
        ########## 我们慢慢地升lockin的电压
        current_volt = self.convert_str_2_float(self.voltage)
        steps = 200
        volt_per_step = (voltage - current_volt) / steps
        next_volt = current_volt + volt_per_step
        for i in range(steps):
            self.voltage = next_volt
            current_volt = next_volt
            next_volt = current_volt + volt_per_step
        ...
        self.harmonic = harmonic_mode

        ########## Ametek7270重新设置电压之后要预读两次，因为前两次的数值会不准
        time.sleep(2.0)
        x, y, m, t = self.read_lockin_levels()
        x, y, m, t = self.read_lockin_levels()
        x, y, m, t = self.read_lockin_levels()

    def convert_str_2_float(self, str_):
        value_ = 0.0
        for i in range(len(str_)):
            if str_[i] == '\n':
                value_ = float(str_[0:i])
    ...
    def read_lockin_levels(self):
        ...
        m = self.convert_str_2_float(self.mag)
        x = self.convert_str_2_float(self.x)
        y = self.convert_str_2_float(self.y)
        if x == 0:
            if y >= 0:
                t = 90
            else:
    ...
    def save_data_2_backward_datfile(self):
        ###################################################################################################
        ########## 将level_bins_forward_cache中的所有数据保存到文件中
        self.put_cache_2_backward_list()
