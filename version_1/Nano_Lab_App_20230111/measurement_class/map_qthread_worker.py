...
# import sys
# sys.path.append('D:/Nano_Lab_App/Nano_Lab_App_20221208/utilities')
...
class map_qthread_worker(QThread):
    ...
    def __init__(self, test_id:int, slow_obj_list: list, fast_obj_list: list, lockin_obj_list: list, user_info: dict, parent=None):
        super(map_qthread_worker, self).__init__(parent)
        ########## 设置工作状态与初始指标
        ...
        self.multi_threads_scan_fast = []
        self.multi_threads_scan_slow = []
        self.multi_threads_lockin = []
    ...
    def meas_function_both_direction(self, flag_rewrite: bool = True):
        ...
        for i in range(self.scan_slow_dims):  # 这个Loop仅供测试使用
            ...
            update_info_list = [self.user_info["test_name"], "forward", rm_time, progress, self.current_i, self.slow_obj_list[0].target_levels_list[self.current_i], self.test_id]
            self.update_sinout.emit(update_info_list)
            ...
            progress = (self.current_i + 1) / self.scan_slow_dims
            update_info_list = [self.user_info["test_name"], "backward", rm_time, progress, self.current_i, self.slow_obj_list[0].target_levels_list[self.current_i], self.test_id]
            self.update_sinout.emit(update_info_list)
    ...
    def set_level_with_cache_clean_forward(self):
        ...
        #         obj.read_level_forward(self.current_i)
        #
        # time.sleep(2.0)  # 让线程休眠3秒, 不要休息太长时间,因为都用了set_level_fast
        # ########## 读取lockin
        # for obj in self.lockin_obj_list:
        #     ########## 快速设置之后读几次lockin
        ...
        for obj in self.fast_obj_list:
            self.multi_threads_scan_fast.append(
                threading.Thread(target=self.__set_level_with_cache_clean_for_one_fast_obj_forward, args=(obj,))
            )

        for obj in self.slow_obj_list:
            self.multi_threads_scan_fast.append(
                threading.Thread(target=self.__set_level_with_cache_clean_for_one_slow_obj_forward, args=(obj,))
            )

        for thread in self.multi_threads_scan_fast:
            thread.daemon = True
            thread.start()
        ...
        for obj in self.lockin_obj_list:
            self.multi_threads_lockin.append(
                threading.Thread(target=self.__read_for_one_lockin_obj_forward_first, args=(obj,))
            )

        for thread in self.multi_threads_lockin:
            thread.daemon = True
            thread.start()
    ...
    def set_level_slow_for_all_obj_forward(self):
        ...
        for obj in self.fast_obj_list:
            self.multi_threads_scan_fast.append(
                threading.Thread(target=self.__set_level_slow_for_one_obj_forward, args=(obj,))
            )

        for obj in self.slow_obj_list:
            self.multi_threads_scan_fast.append(
                threading.Thread(target=obj.read_level_forward, args=(self.current_i, ))
            )
        ...
        for obj in self.lockin_obj_list:
            self.multi_threads_lockin.append(
                threading.Thread(target=self.__read_for_one_lockin_obj_forward, args=(obj,))
            )
    ...
    def __set_level_slow_for_one_obj_forward(self, obj):
        obj.set_level_slow_forward(self.current_j)
        obj.read_level_forward(self.current_j)

    def __set_level_with_cache_clean_for_one_fast_obj_forward(self, obj):
        obj.clean_forward_cache()
        target_level_start = obj.target_levels_list[self.current_j]
        obj.set_level_fast(target_level=target_level_start)
        obj.read_level_forward(self.current_j)

    def __set_level_with_cache_clean_for_one_slow_obj_forward(self, obj):
        ########## 清空cache
        if self.current_i == 0:
            obj.clean_forward_cache()
            obj.set_level_fast(target_level=obj.target_levels_list[0])
    ...
    def __read_for_one_lockin_obj_forward(self, obj):
        obj.read_lockin_levels_forward()

    def __read_for_one_lockin_obj_forward_first(self, obj):
        ########## 快速设置之后读几次lockin
        x, y, m, t = obj.read_lockin_levels()
        time.sleep(0.5)
        x, y, m, t = obj.read_lockin_levels()
    ...
    def set_level_with_cache_clean_backward(self):
        ...
        # for obj in self.fast_obj_list:
        #     self.multi_threads_scan_fast.append(
        #         threading.Thread(target=self.__set_level_with_cache_clean_for_one_fast_obj_backward, args=(obj,))
        #     )
        #
        # for obj in self.slow_obj_list:
        #     self.multi_threads_scan_fast.append(
        #         threading.Thread(target=self.__set_level_with_cache_clean_for_one_slow_obj_backward, args=(obj,))
        #     )
        #
        # for thread in self.multi_threads_scan_fast:
        #     thread.daemon = True
    ...
    def set_level_slow_for_all_obj_backward(self):
        ...
        # for obj in self.fast_obj_list:
        #     self.multi_threads_scan_fast.append(
        #         threading.Thread(target=self.__set_level_slow_for_one_obj_backward, args=(obj,))
        #     )
        #
        # for obj in self.slow_obj_list:
        #     self.multi_threads_scan_fast.append(
        #         threading.Thread(target=obj.read_level_backward, args=(self.current_i,))
        #     )
        #
        # for thread in self.multi_threads_scan_fast:
        #     thread.daemon = True
        ...
        # for obj in self.lockin_obj_list:
        #     self.multi_threads_lockin.append(
        #         threading.Thread(target=self.__read_for_one_lockin_obj_backward, args=(obj,))
        #     )
        #
        # for thread in self.multi_threads_lockin:
        #     thread.daemon = True
    ...
    def __set_level_slow_for_one_obj_backward(self, obj):
        obj.set_level_slow_backward(self.current_j)
        obj.read_level_backward(self.current_j)

    def __set_level_with_cache_clean_for_one_fast_obj_backward(self, obj):
        obj.clean_backward_cache()
        target_level_start = obj.target_levels_list[self.current_j]
        obj.set_level_fast(target_level=target_level_start)
        obj.read_level_backward(self.current_j)

    def __set_level_with_cache_clean_for_one_slow_obj_backward(self, obj):
        obj.clean_backward_cache()
        obj.set_level_slow_backward(self.current_i)
        obj.read_level_backward(self.current_i)

    def __read_for_one_lockin_obj_backward(self, obj):
        obj.read_lockin_levels_backward()

    def __read_for_one_lockin_obj_backward_first(self, obj):
        ########## 快速设置之后读几次lockin
        x, y, m, t = obj.read_lockin_levels()
        time.sleep(0.5)
        x, y, m, t = obj.read_lockin_levels()
